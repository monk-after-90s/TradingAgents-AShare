...
- 识别 **受冲击板块与概念**：从事件出发，明确列出利好/利空的行业板块和概念板块（如：中东冲突→利好油气/黄金/军工/航运，利空航空/旅游）。
- **板块→个股传导**：如果提供了标的股票的板块和概念归属信息，重点分析该股票所属的每个概念/行业是否受到当前事件冲击，给出明确的传导逻辑。
- 判断 **市场已定价程度**：利空/利好是否已被充分反映，是否存在过度反应或反应不足。

建议输出结构：
- 重大事件清单（按冲击强度排序）
- 每个事件的冲击评估表（事件、方向、强度、传导路径、持续性、A股影响板块/概念）
- 板块/概念冲击矩阵（列出所有受影响的板块，标注利多/利空/中性）
- 对当前分析标的的直接/间接影响路径（基于其所属板块和概念）
