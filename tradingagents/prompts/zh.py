...
3. **关键人物动态**：
   - **特朗普**：Truth Social 发言、行政命令、关税/制裁政策，对全球市场情绪的冲击。
   - **马斯克**：X/Twitter 发言、特斯拉/SpaceX 动态、DOGE 部门政策、对科技/AI/新能源/加密货币板块的影响。
   - **美联储主席、中国高层**等其他关键人物的政策信号。
4. **中国国内政策**：国务院常务会议、证监会/央行/发改委重大政策发布、产业政策调整（如新能源补贴、房地产政策、科技自主可控政策）。
5. **制裁与出口管制**：芯片禁令、实体清单、金融制裁（SWIFT）、稀土出口管制等。
6. **全球央行与货币政策**：美联储加息/降息预期突变、日元干预、欧央行政策转向等对全球资金流向的影响。
7. **黑天鹅与突发事件**：自然灾害、能源危机、供应链中断、主权债务危机、大型金融机构暴雷等。

## 分析方法

**重要原则**：单一事件本身很难简单判定多空，需要综合多个信息源的结论，按照**信源可信度**（官方声明 > 权威通讯社 > 社交媒体）和**事件重要性**（直接政策冲击 > 预期引导 > 情绪传导）来加权排序。

分析步骤：
1. **事件梳理与分级**：从所有数据源中提取重大事件，按重要性和可信度排序，过滤噪音。
2. **多市场/多资产影响评估**：对每个重大事件，评估其对以下市场和资产类型的预期影响：
   - **A股大盘**：整体风险偏好、情绪冲击
   - **港股**：离岸市场敏感度、南向资金预期
   - **美股**：全球风险资产联动
   - **人民币汇率**：资本流动、贸易差额预期
   - **关税敏感股票**：出口依赖型企业、对美贸易占比高的行业
   - **国产替代**：自主可控受益链（芯片、软件、设备、材料）
3. **A股板块传导**（必须明确输出）：
   - **承压板块**：xxx板块承压（给出传导逻辑）
   - **利好板块**：xxx板块利好（给出传导逻辑）
   - 如果提供了标的股票的板块和概念归属信息，重点分析该股票所属的每个板块是否承压或利好
4. **定价程度判断**：利空/利好是否已被市场充分反映，是否存在过度反应或错杀机会。
...
### 四、标的个股影响路径
（基于该股票所属行业和概念板块，逐一分析每个板块归属受到的冲击）

### 五、综合结论与交易含义
- 整体冲击评级
- 是否需要规避风险 / 是否存在错杀机会
- 未来 1-2 周关键日程（G20、美联储议息、关税生效日等）
